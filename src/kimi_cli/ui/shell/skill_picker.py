...
Provides three full-screen prompt_toolkit Applications:
- SkillMenuApp: choose between listing/running and managing enablement
- SkillListApp: browse skills and select one to run
- SkillEnableApp: toggle skills on/off
"""
...
class SkillMenuApp:
    ...
    _MENU_ITEMS = [
        ("list", "List & run skills"),
        ("manage", "Manage skill enablement"),
    ]
    ...
    async def run(self) -> str | None:
        """Run the menu and return 'list', 'manage', or None if cancelled."""
    ...
    def _header_fragments(self) -> StyleAndTextTuples:
        return [
            ...
            ("class:header.meta", " Select an action "),
        ]
    ...
    def _footer_fragments(self) -> StyleAndTextTuples:
        return [
            ...
            ("class:footer.text", " \u00b7 "),
            ("class:footer.text", "Esc to exit "),
        ]

    def _list_fragments(self) -> StyleAndTextTuples:
        fragments: StyleAndTextTuples = []
        for idx, (_key, label) in enumerate(self._MENU_ITEMS):
    ...
    def _select_current(self) -> None:
        self._result = self._MENU_ITEMS[self._selected_index][0]
...
class SkillListApp:
    ...
    async def run(self) -> str | None:
        """Run the picker and return the selected skill name or None if cancelled."""
        await self._app.run_async()
        return self._result

    def _header_fragments(self) -> StyleAndTextTuples:
        return [
            ("class:header.title", " LIST SKILLS "),
            ("class:header.meta", f" [{len(self._skills)} total] "),
        ]
    ...
    def _footer_fragments(self) -> StyleAndTextTuples:
        return [
            ...
            ("class:footer.text", "Up/Down to navigate"),
            ("class:footer.text", " \u00b7 "),
            ("class:footer.text", "Enter to run"),
            ("class:footer.text", " \u00b7 "),
            ("class:footer.text", "Esc to exit "),
        ]
    ...
    def _list_fragments(self) -> StyleAndTextTuples:
        ...
        for idx, skill in enumerate(self._skills):
            ...
            if status:
                fragments.append(("class:disabled", status))
            if desc:
...
class SkillEnableApp:
    ...
    def _header_fragments(self) -> StyleAndTextTuples:
        ...
        return [
            ("class:header.title", " ENABLE/DISABLE SKILLS "),
            (
                "class:header.meta",
